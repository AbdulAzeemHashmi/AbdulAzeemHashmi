...
> ⚠️ **Setup needed:** the snake image will only appear once a small GitHub Action runs in this repo to generate it on an `output` branch. It does not work just by pasting the image tag. Add a file at `.github/workflows/snake.yml` in this profile repo with the workflow shown below, then push it, the action creates the `output` branch automatically within a minute or two and the animation will show up. The same applies if the trophy image above stays broken for a long time, the free endpoint is being shared by thousands of profiles and can time out.
